...
console.log(
    chalk.cyan.bold(
        'NOTE: Create Openfin React App supports TypeScript, Sass, CSS Modules and more without ejecting: '
    )
);
console.log();
...
inquirer
    ...
    .then(answer => {
        ...
        const folders = [
            'config', 'config/jest',
            'scripts', 'scripts/routers', 'scripts/utils',
        ];
        ...
        if (appPackage.devDependencies) {
            // We used to put react-openfin-scripts in devDependencies
            ...
            try {
                ...
                content =
                    ...
                    // Remove react-scripts types
                        .replace(
                            /^\s*\/\/\/\s*<reference\s+types.+?"react-openfin-scripts".*\/>.*(?:\n|$)/gm,
                            ''
                        )
                        .trim() + os.EOL;
